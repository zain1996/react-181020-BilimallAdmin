...
class StandardTable extends PureComponent {
  ...
  getInfo(isOpen,val){
    if(isOpen){
      this.props.dispatch({
        ...
        this.setState({
          expandedRowKeys:[val.OrganizationId]
        })
      });
    }else{
      this.setState({
  ...
  setGroupBuyingMode(val){
    localStorage.setItem('GroupBuyingMode',val);
  }
  render() {
    const {data: {List}, loading,paginationProps} = this.props;
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text,record,index) => <Link onClick={(event)=>{ this.setGroupBuyingMode(record.GroupBuyingMode);this.stop(event)}} to={`/group/groupList&/groupInfo/${record.OrganizationId}`}>{text}</Link>
      },
      {
        title: '省市',
    ...
    const userInfo = ()=>{
      ...
      return  (
        <div>
          ...
          <div>
            <span>身份证正反面照片：</span>
            <span><a target="_bank" href={this.authorization(data.ManagerIdCardFront)}>查看</a>&nbsp;&nbsp;<a target="_bank" href={this.authorization(data.ManagerIdCardBack)}>查看</a></span>
          </div>
