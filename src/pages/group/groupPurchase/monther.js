...
export default class PurchaseMonther extends Component {
  ...
  getData(){
    ...
    dispatch({
      type:'groupPurchase/queryPurchaseMontherList',
      payload:{
  ...
  monthPickerChange(dataMoment,val){
    const {dispatch} = this.props;
    const {OrganizationId} = this.props
    if(!dataMoment){
      return
    ...
    const Monther= moment(val).format('MM')
    this.setState({
      Year:Year,
      Month:Monther,
      current:1
  ...
  render(){
    const {purchase:{montherData:{List,Count},loading}} = this.props
    const date = ()=>{
      return (
        <div style={{marginBottom:24}}>
          <MonthPicker placeholder="选择日期" defaultValue={moment(`${this.state.Year}-${this.state.Month}`, 'YYYY-MM')} onChange={this.monthPickerChange.bind(this)} />
        </div>
      );
    }
