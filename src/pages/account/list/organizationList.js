...
export default class organization extends PureComponent {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'account/queryOrganizationList',
      payload:{
        page:1,
        page_size:10,
      },
  ...
  render() {
    const {account: {organizationList:{List,Count}}, loading} = this.props;
    ...
    return (
        <Card
          extra={extraContent}
          title={<div style={{display:'flex',alignItems:'center',height:'100%'}}>组织成员列表</div>}
          bordered={false}
        >
            <Table
              rowKey="UserID"
