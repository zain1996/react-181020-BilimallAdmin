...
export default class CityListTable extends PureComponent {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'account/queryCommunityList',
      payload:{
        page:1,
        page_size:10,
      },
  ...
  render() {
    const {account: {communityList:{List,Count}}, loading} = this.props;
    ...
    const columns = [
      ...
      },
      {
        title: '操作',
        render: (text, record, index) => (
          <Switch defaultChecked={!record.IsDisableCmtManger} checkedChildren="开启" onChange={(val)=>{this.change(val,record)}} unCheckedChildren="禁用"  />
        ),
      },
    ];
    ...
    return (
        <Card
          extra={extraContent}
          title={<div style={{display:'flex',alignItems:'center',height:'100%'}}>社区合伙人列表</div>}
          bordered={false}
        >
            <Table
              rowKey="UserID"
